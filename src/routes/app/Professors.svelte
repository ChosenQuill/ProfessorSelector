	...
	$: {
		if(sortedBy != 'Difficulty') {
			info = info.sort((prof1, prof2) => {
				const prof1r = prof1.rmp ? Number(sortedMap[sortedBy](prof1)) : -1;
				const prof2r = prof2.rmp ? Number(sortedMap[sortedBy](prof2)) : -1;
			...
			<div class="stats -ml-2 mt-0 bg-base-200">
				...
				<div class="stat">
					...
					<div class="stat-desc text-secondary">
						out of {prof.rmp.numRatings ? prof.rmp.numRatings : '?'} users
					</div>
				</div>
