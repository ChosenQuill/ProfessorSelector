...
<script>
    ...
    const addClass = async () => {
        ...
        if(res) {
            ...
            const found = Object.values(courses).findIndex((course) => course.section == section && course.number == number);
            if(found != -1) {
            ...
            try {
                ...
                if(json.data) {
                    ...
                    courses[id] = {section, number, id, course: json.data[0], professors: []};
                    courses = courses;
                    ...
                    const res3 = await fetch(`/api/professor?cid=${id}`);
                    if(fres?.status != 200) {
          ...
          <div class="flex flex-row w-full">
            <ul class="menu p-4 pr-2 w-full mt-2">
                ...
                <!--  href=`./${c.name}.${c.section}` -->
                    <li><button id={id} href="/app/class/{id}" class:active={(id === current)} class="mb-2" on:click={()=>current = id}>{`${courses[id].section} ${courses[id].number}`}</button></li>
                {/each}
            ...
            <div class="mt-2 pr-2 py-4 w-14 flex flex-col">
                {#each Object.keys(courses) as id (id)}
                    <button class="btn btn-square btn-outline mb-2" style="border: none;" on:click={() => removeClass(id)}>
