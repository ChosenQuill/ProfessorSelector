...
async function getClassData(section: string, number: string) {
...
function parseProfessors(classData): Array<Array<String>> {
    ...
    // @ts-ignore
    const professors: Array<Array<String>> = [...new Set(classData.sections.map((s) => s.instructor[0].name))].map(name => name.split(' '));
    return professors
}
...
export async function getProfessorRating({ name, id }: GPRParams) {
    if (name) {
...
async function main() {
    ...
    while (true) {
        ...
        const { section, number }: { section: string, number: string } = await prompt.get(["section", "number"]);
        try {
            ...
            for (const prof of professors) {
                ...
                try {
                    ...
                    if (!data) {
                        ...
                        const { url }: { url: string } = await prompt.get(["url"])
